...
local function replacement() S=3 cur=3 BT.button(false,false,true,false) end
...
local function turn(move)
  ...
  elseif me.hp==0 then
    push(P[me.id][1].."\nfainted!",nil,"lose")
    local o=en.id==5 and others()
    if o and #o>0 then f=replacement
...
function BT.button(up,dn,A,B)
  ...
  elseif S==5 then
    ...
    elseif A then
      local i=o[cur]
      local fainted=en.id==5 and me.hp==0
